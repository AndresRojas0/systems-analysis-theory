...
Terminada la construccion del mapa canónico.

### *Objetivos*
Es una herramienta gráfica que permite:
1. Controlar la consistencia y coherencia del modelo de datos y el modelo ambiental.
2. Comprender cual es la lógica de obtención de los datos a través de las entidades, para
luego facilitar la construcción de los detalles de los procesos.

El subesquema describe que:
• Datos de las entidades que se usan
• Relaciones entre entidades facilitan el acceso a los datos
• Condiciones internas del sistema que filtran las filas resultantes.
• Condiciones externas al sistema que filtran las filas resultantes.
Se realiza por lo menos un subesquema de navegación por cada evento que dispare una actividad
fundamental o mixta.

### *Procedimiento*
1. Detectar los eventos que disparan actividades fundamentales y/o mixtas.
2. De las actividades mixtas identificar solo la porción fundamental. No tener en cuenta la porción
custodial.
3. Estudiar que entidades, relaciones y datos deberían, en un principio, ser utilizados.
4. Decidir la entidad de comienzo.
5. Decidir las entidades por las que se continua
6. Estudiar que datos permiten "navegar" de una entidad a otra.
7. Determinar que datos se utilizan de cada entidad.
8. Establecer las condiciones externas.
9. Establecer las condiciones internas.

**Navegar** de una entidad a otra significa que puedo obtener ocurrencias en donde coinciden uno o
más datos de una entidad origen con uno o más datos en una entidad destino; o bien, coincide el
resultado de un cálculo en base a dato/s de una entidad origen con dato/s en una entidad destino.

**Ejemplo:**
a) Fragmento de la Narrativa:
“Cada vez que Gerencia lo solicite, el sistema deberá liquidarle las comisiones por venta a los
vendedores, teniendo en cuenta el rango de fechas indicado. Todas las facturas que utilice en
el cálculo deberán ser actualizadas como “liquidadas”. La liquidación deberá tener el siguiente
formato, para cada vendedor:

*Figura 250*

**Flujos**

e: Período a liquidar = fecha desde + fecha hasta

s: Liquidación = 1{fecha liquidación + fecha desde + fecha hasta + código vendedor + nombre +
apellido + 0{código producto + nombre producto + venta + porcentaje + comision}n + total
comision

Elementos discretos:
Estado_liquidacion = “a liquidar”, “liquidada”
...
c) Construcción:
Determinar entonces por cuál de las entidades se tendrá que comenzar. Si observamos un poco
el flujo de salida del evento, veremos que el sistema deberá generar una liquidación para cada
vendedor (y para todos ellos), por lo que podemos graficar esta situación de la siguiente
manera:

*Figura 252*

Se dibuja la entidad, junto con el acceso a la misma. En este caso no vamos a trabajar con un
vendedor determinado o un rango de vendedores, es decir: no accederemos a VENDEDORES
con ningún atributo o grupo de atributos, sino que recuperaremos todas las ocurrencias de esta
entidad. Por esa razón la flecha solo indica el comienzo.

Además, se van a recuperar datos del vendedor que deberán aparecer en el flujo de salida,
esta situación la graficamos de la siguiente manera:

*Figura 253*

De todas las facturas de cada vendedor, sólo nos van a interesar aquellas comprendidas dentro
del rango de fechas indicado por Gerencia y que no estén liquidadas. Esta situación la
graficamos de la siguiente manera:

*Figura 254*

La línea punteada representará un filtro para la entidad con datos originados en forma interna
por el sistema (condición interna), mientras que el rango de fechas también será un filtro pero
con datos originados en el ambiente (condición externa) y se graficará con una línea llena.

No hay ningún dato de FACTURAS que se necesite en el flujo de salida, pero sí será necesario
navegar hacia PRODUCTOS POR FACTURA, para poder ir calculando el total vendido, agrupado
por cada producto:

*Figura 255*

Por último, debemos recuperar el porcentaje y el precio necesario para liquidar la comisión para
cada producto, por lo que nuestro sub-esquema de navegación quedará finalmente:
