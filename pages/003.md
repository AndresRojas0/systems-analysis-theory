...
## Herramientas que se usan en la construcción de una mini especificación:
...
Por supuesto, será necesario que esta notación se emplee sólo cuando sea necesario, es decir: para 
evitar ambigüedades en la miniespecificación.
...
[ ] Para indicar que esa palabra es opcional dentro de la sentencia;
{construcción 1; construcción 2; ...; construcción n } para indicar que debe optarse por una de las
construcciones listadas;
...
siendo inscripción al club = nombre + domicilio + teléfono + fax + e-mail + fecha nacimiento + código categoría
...
siendo inhabilitación por rubro = código rubro
...
siendo novedades socio = número socio + (nombre) + (domicilio) + (teléfono) + (fax) + (e-mail) + (fecha nacimiento)
...
- presupuesto a consultar = número presupuesto y
